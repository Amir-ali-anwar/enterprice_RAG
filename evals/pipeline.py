...
def _summarize_for_eval(answer: str, question: str) -> str:
    ...
    Preserves specific numbers, names, and technical claims — the things RAGAS
    judges on.

    Falls back to a 600-character slice if Groq is unavailable.
    """
    if not answer or len(answer) <= SUMMARIZE_THRESHOLD:
        return answer

    prompt = (
        "Summarize the following answer to the question in 3-5 bullet points.\n"
        "Rules: preserve ALL specific facts, numbers, names, and technical details. "
        "Do NOT add information not present in the answer. Be concise.\n\n"
        f"Question: {question}\n\n"
        f"Answer: {answer}\n\n"
    ...
    try:
        ...
        logfire.info(
            f"📝 Summarized answer: " f"{len(answer)} chars → {len(summary)} chars"
        )
        ...
        for i, sample in enumerate(samples):
            ...
            with logfire.span(
                ...
                try:
                    resp = requests.post(
                        API_URL,
                        json={"query": question, "thread_id": f"eval_run_{i}"},
                        timeout=60,
                    )
                    ...
                    sample["actual_response"] = _summarize_for_eval(
                        raw_answer, question
                    )
                    sample["actual_contexts"] = sources[:5]
                    sample["actual_tools_called"] = [_detect_tool(thought_process)]
                    logfire.info(
                        "Response captured",
                        tool=sample["actual_tools_called"],
