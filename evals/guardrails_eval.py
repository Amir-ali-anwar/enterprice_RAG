...
def _is_blocked(response_json: dict) -> bool:
    thought_process = response_json.get("thought_process") or []
    return any("guardrails fired" in step.lower() for step in thought_process)
        ...
        for i, sample in enumerate(samples):
            ...
            with logfire.span(
                ...
                try:
                    resp = requests.post(
                        API_URL,
                        json={"query": sample["input"], "thread_id": f"guardrail_eval_{i}"},
                        timeout=30,
                    )
...
run_guardrail_eval = run_guardrails_eval
